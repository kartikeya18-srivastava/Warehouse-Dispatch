...
export const completeDeliveryController =
    async (req: Request, res: Response): Promise<void> => {
        const { shipmentId } = req.params as { shipmentId: string };
        const { signatureUrl, photoUrl } = req.body;

        if (!shipmentId) {
            throw new AppError("Shipment ID required", 400);
        }

        const delivery = await completeDeliveryService(
            shipmentId,
            signatureUrl,
            photoUrl
        );
...
export const reportExceptionController =
    async (req: Request, res: Response): Promise<void> => {
        const { shipmentId } = req.params as { shipmentId: string };
        ...
        const delivery = await reportExceptionService(
            shipmentId,
            exception
        );
